...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onDrawForeground = function (ctx) {
            ...
            const ipw = itw+10, iph = 14, ipx = 8, ipy = 2;
            ...
            // Widget slot Y helper — LiteGraph stacks widgets after title
            const TH      = LiteGraph.NODE_TITLE_HEIGHT;
            const wH      = LiteGraph.NODE_WIDGET_HEIGHT ?? 20;
            const gap     = 4;
            const slotY   = (idx) => TH + 4 + idx*(wH+gap) + wH/2 - 7;

            // Widget order: clip_name_1(0), clip_type_1(1), load_mode_1(2),
            //               dual_clip(3), clip_name_2(4), clip_type_2(5), load_mode_2(6)
            drawPill(mode1, slotY(2));

            // SINGLE / DUAL indicator beside the toggle (slot 3)
            ctx.font="bold 10px sans-serif"; ctx.textAlign="left";
            ctx.textBaseline="middle";
            ctx.fillStyle    = isDual ? "#f5a623" : "#7a5a20";
            ctx.shadowColor  = isDual ? "#f5a623" : "transparent";
            ctx.shadowBlur   = isDual ? 5 : 0;
            ctx.fillText(isDual ? "◉ DUAL" : "◎ SINGLE", 12, TH+4+3*(wH+gap)+wH/2);

            if (isDual) {
                ctx.shadowBlur=0; ctx.shadowColor="transparent";
                drawPill(mode2, slotY(6));
            }
